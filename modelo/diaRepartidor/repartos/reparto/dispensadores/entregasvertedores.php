...
include_once($_SERVER["DOCUMENT_ROOT"] . '/AplicacionSM/modelo/diaRepartidor/repartos/reparto/dispensadores/entregavertedor.php');




class EntregasVertedores extends GenericoLista
{

  function __construct($idRepartidor=null,$fecha=null)
  {
  parent::__construct();
  $this->nombreItem="EntregaVertedor";
  if($idRepartidor!=null && $fecha!=null)
    {
    if(parent::abrirConexion())
      {

      $sql = "SELECT * FROM entregavertedores WHERE fecha='$fecha' AND idrepartidor='$idRepartidor'";
      ...
      if($tabla->num_rows>0)
          ...
          while($row = $tabla->fetch_assoc())
            {
            $this->lista[$k] = new EntregaVertedor($row["idcliente"],$row["iddireccion"],$row["idrepartidor"],$fecha);
            $k++;
            }
          $this->size=$k;
          }
