...
include_once($_SERVER["DOCUMENT_ROOT"] . '/AplicacionSM/modelo/diaRepartidor/repartos/reparto/dispensadores/cambiovertedor.php');




class CambiosVertedores extends GenericoLista
{

  function __construct($idRepartidor=null,$fecha=null)
  {
  parent::__construct();
  $this->nombreItem="CambioVertedor";
  if($idRepartidor!=null && $fecha!=null)
    {
    if(parent::abrirConexion())
      {

      $sql = "SELECT * FROM cambiovertedores WHERE fecha='$fecha' AND idrepartidor='$idRepartidor'";
      ...
      if($tabla->num_rows>0)
          ...
          while($row = $tabla->fetch_assoc())
            {
            $this->lista[$k] = new CambioVertedor($row["idcliente"],$row["iddireccion"],$row["idrepartidor"],$fecha);
            $k++;
            }
          $this->size=$k;
          }
