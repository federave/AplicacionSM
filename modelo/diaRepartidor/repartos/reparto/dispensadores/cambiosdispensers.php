...
include_once($_SERVER["DOCUMENT_ROOT"] . '/AplicacionSM/modelo/diaRepartidor/repartos/reparto/dispensadores/cambiodispenser.php');




class CambiosDispensers extends GenericoLista
{

  function __construct($idRepartidor=null,$fecha=null)
  {
  parent::__construct();
  $this->nombreItem="CambioDispenser";
  if($idRepartidor!=null && $fecha!=null)
    {
    if(parent::abrirConexion())
      {

      $sql = "SELECT * FROM cambiodispensers WHERE fecha='$fecha' AND idrepartidor='$idRepartidor'";
      ...
      if($tabla->num_rows>0)
          ...
          while($row = $tabla->fetch_assoc())
            {
            $this->lista[$k] = new CambioDispenser($row["idcliente"],$row["iddireccion"],$row["idrepartidor"],$fecha);
            $k++;
            }
          $this->size=$k;
          }
