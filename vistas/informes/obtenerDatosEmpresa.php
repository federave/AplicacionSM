...
$xml = new Xml();
$xml->startTag("DatosInformeEmpresa");
...
if($conector->abrirConexion())
  ...
  //////Productos Vendidos////////
    ...
    if($tabla->num_rows>0)
        ...
        $sql = "SELECT alquilerdispenser_bidonesentregados.IdCliente FROM alquilerdispenser_bidonesentregados inner join clientes on clientes.IdCliente=alquilerdispenser_bidonesentregados.IdCliente WHERE mes = '$mes' AND año = '$year'";
        ...
        */
        ...
        if($tablaDP->num_rows>0)
          ...
          $xml->closeTag("DeudasProductos");
                            ...
                            $sql = "select count(distinct(idcliente)) from planilladinamica
                            where fecha between '$fechaInicio' and '$fechaFin' and (NBidon20L>0 OR NBidon12L>0 OR NBidon10L>0 OR NBidon8L>0 OR NBidon5L>0 OR NPackBotellas2L>0 OR NPackBotellas500mL>0 OR NBidon20L_A>0 OR NBidon12L_A>0)";
                            $tabla = $conexion->query($sql);
                            if($tabla->num_rows>0)
                              {
                              $row = $tabla->fetch_assoc();
                            ...
                            $sql = "select idcliente from planilladinamica
                            where fecha between '$fechaInicio' and '$fechaFin' and not(NBidon20L>0 OR NBidon12L>0 OR NBidon10L>0 OR NBidon8L>0 OR NBidon5L>0 OR NPackBotellas2L>0 OR NPackBotellas500mL>0 OR NBidon20L_A>0 OR NBidon12L_A>0)";
                            $tabla = $conexion->query($sql);
...
$xml->closeTag("DatosInformeEmpresa");
