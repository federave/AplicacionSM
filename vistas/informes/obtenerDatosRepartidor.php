...
$xml = new Xml();
$xml->startTag("DatosInformeRepartidor");

$xml->addTag("FechaInicio",$fechaInicio);
$xml->addTag("FechaFin",$fechaFin);
$xml->addTag("Repartidor",$repartidor->toString());
...
if($conector->abrirConexion())
  ...
  //////Productos Vendidos////////
    ...
    $sql = "
          ...
          where fecha between '$fechaInicio' and '$fechaFin' and IdEmpleado = '$idRepartidor';";
    ...
    if($tabla->num_rows>0)
        ...
        $sql = "SELECT alquilerdispenser_bidonesentregados.IdCliente FROM alquilerdispenser_bidonesentregados inner join clientes on clientes.IdCliente=alquilerdispenser_bidonesentregados.IdCliente WHERE mes = '$mes' AND año = '$year' AND clientes.IdEmpleado = '$idRepartidor'";
        ...
        */
        ...
        $sql = "SELECT count(IdDeuda),sum(NBidon20L),sum(NBidon12L),sum(NBidon10L),sum(NBidon8L),sum(NBidon5L),sum(NPackBotellas2L),sum(NPackBotellas500mL),SUM(NBidon20L*PBidon20L+NBidon12L*PBidon12L+NBidon10L*PBidon10L+NBidon8L*PBidon8L+NBidon5L*PBidon5L+NPackBotellas2L*PPackBotellas2L+NPackBotellas500mL*PPackBotellas500mL) FROM Deudas_Productos WHERE Fecha>='$fechaInicio' AND Fecha<='$fechaFin' AND IdEmpleado='$idRepartidor'";
        $tablaDP = $conexion->query($sql);
        if($tablaDP->num_rows>0)
            {
            $rowDP = $tablaDP->fetch_assoc();

              $xml->addTag("Bidones20L",$rowDP["sum(NBidon20L)"]);
              $xml->addTag("Bidones12L",$rowDP["sum(NBidon12L)"]);
              $xml->addTag("Bidones10L",$rowDP["sum(NBidon10L)"]);
              $xml->addTag("Bidones8L",$rowDP["sum(NBidon8L)"]);
              $xml->addTag("Bidones5L",$rowDP["sum(NBidon5L)"]);
              $xml->addTag("PackBotellas2L",$rowDP["sum(NPackBotellas2L)"]);
              $xml->addTag("PackBotellas500mL",$rowDP["sum(NPackBotellas500mL)"]);
              $xml->addTag("DineroTotal",$rowDP["SUM(NBidon20L*PBidon20L+NBidon12L*PBidon12L+NBidon10L*PBidon10L+NBidon8L*PBidon8L+NBidon5L*PBidon5L+NPackBotellas2L*PPackBotellas2L+NPackBotellas500mL*PPackBotellas500mL)"]);
              $xml->addTag("Numero",$rowDP["count(IdDeuda)"]);
          ...
          $xml->closeTag("DeudasProductos");
                  ...
                  where idempleado='$idRepartidor' and fecha between '$fechaInicio' and '$fechaFin'";
                  $tabla = $conexion->query($sql);
                  if($tabla->num_rows>0)
                    {
                    $row = $tabla->fetch_assoc();
                  ...
                  $sql = "select count(distinct(idcliente)) from planilladinamica
                  where idempleado='$idRepartidor' and fecha between '$fechaInicio' and '$fechaFin' and (NBidon20L>0 OR NBidon12L>0 OR NBidon10L>0 OR NBidon8L>0 OR NBidon5L>0 OR NPackBotellas2L>0 OR NPackBotellas500mL>0 OR NBidon20L_A>0 OR NBidon12L_A>0)";
                  $tabla = $conexion->query($sql);
                  if($tabla->num_rows>0)
                    {
                    $row = $tabla->fetch_assoc();
                  ...
                  $sql = "select idcliente from planilladinamica
                  where idempleado='$idRepartidor' and fecha between '$fechaInicio' and '$fechaFin' and not(NBidon20L>0 OR NBidon12L>0 OR NBidon10L>0 OR NBidon8L>0 OR NBidon5L>0 OR NPackBotellas2L>0 OR NPackBotellas500mL>0 OR NBidon20L_A>0 OR NBidon12L_A>0)";
                  $tabla = $conexion->query($sql);
...
$xml->closeTag("DatosInformeRepartidor");
